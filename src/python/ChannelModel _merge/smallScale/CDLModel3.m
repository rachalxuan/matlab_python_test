function [H_t,tao_n,P_n,dopplerFreqCenter] = CDLModel3(parameter,position)
...
H_Martix(:,:,len:end) = sum(H_usnmNlos, 4);

% 同样，对时变信道系数进行向量化求和
% H_usnmNlos_t 尺寸为 [N_r, N_t, comPara.N, scenariosPara.M, length_t]
% 沿第4维求和后得到 [N_r, N_t, comPara.N, length_t]，赋值到 H_Martix_t 对应位置
H_Martix_t(:,:,len:end,:) = sum(H_usnmNlos_t, 4);
...
F_txfield = reshape(F_txfield,[2,N,M]);                         % 2026_6_6
Knm = modePara.Knm;
