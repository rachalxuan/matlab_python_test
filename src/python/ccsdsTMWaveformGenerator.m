...
function localValidateHighRateConvFrameLength(obj)
    ...
    % setupImpl@tmBase has already calculated the actual inner-encoder
    ...
    if mod(caduBits, punctureInputPeriod) ~= 0
        if strcmp(obj.ChannelCoding,'concatenated')
            error('ccsdsTMWaveformGenerator:HighRateConvFrameLengthUnsupported', ...
                ['Concatenated ConvolutionalCodeRate="%s" requires ASM+RS ', ...
                 'encoded length (%d bits) to be divisible by %d. ', ...
                 'Use a compatible inner code rate (for example 1/2) or ', ...
                 'an explicitly validated RS profile; changing the inactive ', ...
                 'NumBytesInTransferFrame property cannot fix this.'], ...
                char(obj.ConvolutionalCodeRate), caduBits, punctureInputPeriod);
        end
        error('ccsdsTMWaveformGenerator:HighRateConvFrameLengthUnsupported', ...
