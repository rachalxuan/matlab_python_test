...
function record = localEmptyRecord()
    ...
    diagnosticTextFields = localDiagnosticTextResultFields();
    for k = 1:numel(diagnosticTextFields)
        record.(diagnosticTextFields{k}) = "";
    end
end
...
function record = localFinishRecord(record, actual, verdict, elapsed)
    ...
    diagnosticTextFields = localDiagnosticTextResultFields();
    for k = 1:numel(diagnosticTextFields)
        name = diagnosticTextFields{k};
        record.(name) = localText(actual,name,"");
    end
...
function T = localResultsTable(records)
    ...
    diagnosticTextFields = localDiagnosticTextResultFields();
    for j = 1:numel(diagnosticTextFields)
        T.(diagnosticTextFields{j}) = strings(n,1);
    end
    for k = 1:n
...
function names = localDiagnosticNumericResultFields()
    names = { ...
        ...
        'QAMPowerGainApplied','QAMPowerGainAcceptanceRate', ...
        ...
        'BlindReliabilityApplied', ...
        'BlindReliabilityWindowSymbols', ...
        'BlindReliabilityHoldFraction', ...
        'BlindReliabilityHoldEvents', ...
...
function names = localDiagnosticTextResultFields()
    names = { ...
        'SplitReceiverStructureSelectionMethod', ...
        'UQPSKSymSkipSelectionMethod', ...
        'PilotlessAPSKRingEstimatorMode', ...
        'BlindReliabilityReason', ...
        'BlindReliabilityFinalState'};
end
...
function [records, compactedCount] = localCompactResultRecords(records)
    ...
    for k = 1:numel(records)
        ...
        diagnosticTextFields = localDiagnosticTextResultFields();
        for j = 1:numel(diagnosticTextFields)
            name = diagnosticTextFields{j};
            if strlength(string(records(k).(name))) == 0
                records(k).(name) = localText(actual,name,"");
            end
        end
...
function localValidateResumeState(state, suiteName, cases, caseIds, planVersion)
    ...
    [storedComparable,currentComparable] = ...
        localNormalizeResumeCompatibleCases(state.Cases,cases);
    if ~isequaln(storedComparable, currentComparable)
        detail = localFirstCaseDefinitionDifference( ...
            storedComparable, currentComparable);
        error('reg_run_cases:CaseDefinitionMismatch', ...
            ['Checkpoint parameters, thresholds, or metadata differ from the ' ...
             'current cases. First difference: %s. Choose a new ', ...
             'OutputDir/RunId, or restore that saved option before resuming.'], ...
            detail);
    end
end

function [storedCases,currentCases] = localNormalizeResumeCompatibleCases( ...
        storedCases,currentCases)
    % Pure diagnostics do not alter the transmitted or received waveform and
    % may be excluded from resume identity. Keep this list deliberately
    % narrow: signal-processing, threshold, coding, seed, channel, and data
    % source differences must still fail the strict comparison below.
    compatibleParamFields = { ...
        'debugCodedBoundary', ...
        'debugTMDataSource', ...
        'debugConvolutionalG1G2'};
    for k = 1:min(numel(storedCases),numel(currentCases))
        if isfield(storedCases(k),'Params') && isstruct(storedCases(k).Params)
            present = compatibleParamFields( ...
                isfield(storedCases(k).Params,compatibleParamFields));
            if ~isempty(present)
                storedCases(k).Params = rmfield(storedCases(k).Params,present);
            end
        end
        if isfield(currentCases(k),'Params') && isstruct(currentCases(k).Params)
            present = compatibleParamFields( ...
                isfield(currentCases(k).Params,compatibleParamFields));
            if ~isempty(present)
                currentCases(k).Params = rmfield(currentCases(k).Params,present);
            end
        end
    end
end

function detail = localFirstCaseDefinitionDifference(storedCases, currentCases)
    detail = 'unknown field';
    n = min(numel(storedCases), numel(currentCases));
    for k = 1:n
        if ~isequaln(storedCases(k), currentCases(k))
            if isfield(storedCases(k),'Params') && ...
                    isstruct(storedCases(k).Params) && ...
                    isfield(currentCases(k),'Params') && ...
                    isstruct(currentCases(k).Params)
                added = setdiff(fieldnames(currentCases(k).Params), ...
                    fieldnames(storedCases(k).Params),'stable');
                if ~isempty(added)
                    detail = sprintf(['case %d (%s), Params fields added ', ...
                        'since checkpoint: %s'],k, ...
                        char(string(storedCases(k).Id)),strjoin(added,', '));
                    return;
                end
            end
            path = localFirstValueDifference(storedCases(k), currentCases(k), 'case');
            detail = sprintf('case %d (%s), %s', k, ...
                char(string(storedCases(k).Id)), char(path));
            return;
        end
    end
    if numel(storedCases) ~= numel(currentCases)
        detail = sprintf('case count stored=%d current=%d', ...
            numel(storedCases), numel(currentCases));
    end
end

function detail = localFirstValueDifference(stored, current, path)
    detail = string(path);
    if isstruct(stored) && isstruct(current) && ...
            isscalar(stored) && isscalar(current)
        storedNames = fieldnames(stored);
        currentNames = fieldnames(current);
        allNames = unique([storedNames; currentNames], 'stable');
        for iName = 1:numel(allNames)
            name = allNames{iName};
            nextPath = string(path) + "." + string(name);
            if ~isfield(stored, name)
                detail = nextPath + " missing from checkpoint";
                return;
            end
            if ~isfield(current, name)
                detail = nextPath + " missing from current plan";
                return;
            end
            if ~isequaln(stored.(name), current.(name))
                detail = localFirstValueDifference( ...
                    stored.(name), current.(name), nextPath);
                return;
            end
        end
        return;
    ...
    detail = string(path) + " stored=" + localCompactValue(stored) + ...
        " current=" + localCompactValue(current);
end

function text = localCompactValue(value)
    ...
    elseif isempty(value)
        text = "[]";
    elseif iscell(value)
        text = "<cell " + string(mat2str(size(value))) + ">";
    elseif isstruct(value)
        text = "<struct " + string(mat2str(size(value))) + ">";
    else
        text = "<" + string(class(value)) + " " + ...
            string(mat2str(size(value))) + ">";
    end
end
