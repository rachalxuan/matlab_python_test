...
function cfg = localConfig(profile, userOpts)
    % IncludeUncoded defaults to true for backward-compatible diagnostic
    % sweeps. Formal coded-link regressions can set it false so an
    % intentionally unsupported uncoded configuration does not count as a
    % coding regression failure.
    cfg = struct( ...
        'Profile', profile, ...
        'OutputDir', '', ...
...
function bytes = localTransferFrameBytes(modulation, coding)
    ...
    elseif strcmpi(coding.Coding, 'convolutional')
        ...
        if strcmp(rate, '3/4') && any(strcmpi(modulation, {'32QAM','32APSK'}))
            bytes = 1121;
    ...
    elseif strcmpi(coding.Coding, 'none') && ...
            any(strcmpi(modulation, {'32QAM','32APSK'}))
        bytes = 1116;
    end
end
...
function T = localAttachMetadata(T, cases, state)
    ...
    T.SplitReceiverImplementation = strings(n,1);
    T.SplitReceiverIQPhase = nan(n,1);
    T.UQPSKSymSkip = nan(n,1);
    T.SplitReceiverStructureScore = nan(n,1);
    T.SplitReceiverTMValidFrames = nan(n,1);
    T.SplitReceiverTMMinValidFrames = nan(n,1);
    T.SplitReceiverTMMaxCounterRun = nan(n,1);
    T.SplitReceiverTMOrientationScore = nan(n,1);
    ...
    for k = 1:n
        ...
        if k <= numel(state.Results)
            ...
            T.SplitReceiverImplementation(k) = localStructString( ...
                record, 'SplitReceiverImplementation', "");
            T.SplitReceiverIQPhase(k) = localStructNumeric( ...
                record, 'SplitReceiverIQPhase', NaN);
            T.UQPSKSymSkip(k) = localStructNumeric( ...
                record, 'UQPSKSymSkip', NaN);
            T.SplitReceiverStructureScore(k) = localStructNumeric( ...
                record, 'SplitReceiverStructureScore', NaN);
            T.SplitReceiverTMValidFrames(k) = localStructNumeric( ...
                record, 'SplitReceiverTMValidFrames', NaN);
            T.SplitReceiverTMMinValidFrames(k) = localStructNumeric( ...
                record, 'SplitReceiverTMMinValidFrames', NaN);
            T.SplitReceiverTMMaxCounterRun(k) = localStructNumeric( ...
                record, 'SplitReceiverTMMaxCounterRun', NaN);
            T.SplitReceiverTMOrientationScore(k) = localStructNumeric( ...
                record, 'SplitReceiverTMOrientationScore', NaN);
            T.SplitReceiverStructureSelectionMethod(k) = localStructString( ...
                record, 'SplitReceiverStructureSelectionMethod', "");
            T.UQPSKSymSkipSelectionMethod(k) = localStructString( ...
    ...
    leading = {'CaseId','Status','Pass','DataPathMode','Modulation','Coding', ...
        ...
        'SplitReceiverImplementation','SplitReceiverIQPhase','UQPSKSymSkip', ...
        'UQPSKSymSkipSelectionMethod', ...
        'SplitReceiverStructureSelectionMethod','SplitReceiverStructureScore', ...
        'SplitReceiverTMValidFrames','SplitReceiverTMMinValidFrames', ...
        'SplitReceiverTMMaxCounterRun','SplitReceiverTMOrientationScore'};
